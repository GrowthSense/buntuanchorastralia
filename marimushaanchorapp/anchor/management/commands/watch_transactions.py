"""Custom watch_transactions command that filters out assets without distribution accounts."""
import asyncio
...
class Command(BaseCommand):
    ...
    This custom version filters out assets that don't have a distribution account
    configured to prevent AttributeError when account is None.
    """
    ...
    async def watch_transactions(self):  # pragma: no cover
        ...
        # Filter out assets without distribution accounts
        valid_accounts = []
        for asset in assets:
            account = rci.get_distribution_account(asset=asset)
            if account is None:
                logger.warning(
                    f"Skipping asset {asset.code} (issuer: {asset.issuer}) - "
                    f"no distribution account configured (distribution_seed is missing)"
                )
            else:
        ...
        if not valid_accounts:
            logger.warning(
                "No assets with distribution accounts found. "
                "Please configure distribution_seed for at least one asset."
            )
            return
        
        logger.info(f"Watching transactions for {len(valid_accounts)} asset(s)")
        await asyncio.gather(
            *[self._for_account(account) for account in valid_accounts]
        )
